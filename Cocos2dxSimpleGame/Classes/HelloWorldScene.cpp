...
bool HelloWorld::init()
{
	...
	do 
	{
		...
		//CC_BREAK_IF(! CCLayerColor::initWithColor( ccc4(123,166,133,255) ) );
		...
		this->schedule( schedule_selector(HelloWorld::gameLogic), 0.2 );
...
{
	CCSprite *target = CCSprite::spriteWithFile("ufo.png", 
		CCRectMake(0,0,50,25) );
	...
	int minDuration = (int)16.0;
	int maxDuration = (int)20.0;
...
void HelloWorld::shootProjectile(){
    
    
	mLocation = CCDirector::sharedDirector()->convertToGL(mLocation);
	...
	float offX = mLocation.x - projectile->getPosition().x;
	float offY = mLocation.y - projectile->getPosition().y;
...
void HelloWorld::ccTouchesMoved (CCSet* touches, CCEvent* event){
	...
	mLocation = touch->locationInView(touch->view());
